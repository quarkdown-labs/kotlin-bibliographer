...
use hayagriva::citationberg::{IndependentStyle, Locale, LocaleCode};
...
// `static CATALOG_STYLES: &[(&str, &str)]`: name -> XML for every style in the
// repository's shared `styles/` directory, generated by `build.rs`. The JVM
// artifact embeds the same files, keeping the catalog byte-identical across
// platforms.
include!(concat!(env!("OUT_DIR"), "/catalog_styles.rs"));
...
/// else is looked up among the embedded catalog styles.
fn resolve_style(style: &str) -> Result<IndependentStyle, String> {
    let xml = if style.trim_start().starts_with('<') {
        style
    } else {
        CATALOG_STYLES
            .iter()
    ...
    IndependentStyle::from_xml(xml).map_err(|e| format!("Invalid CSL style: {e}"))
}
...
"#;
    ...
    /// Reads a style's XML from the shared `styles/` directory, so XML-path
    /// tests exercise the same definitions the catalog embeds.
    fn style(name: &str) -> String {
        std::fs::read_to_string(format!("{}/../styles/{name}.csl", env!("CARGO_MANIFEST_DIR")))
            .unwrap()
    }

    fn ieee() -> String { style("ieee") }

    fn apa() -> String { style("apa") }
    ...
    /// A catalog name must render exactly like the XML file it embeds.
    #[test]
    fn catalog_names_resolve_to_their_style_file() {
    ...
    /// Every vendored style must parse and construct, so a broken or
    /// dependent .csl file cannot sit unnoticed in the catalog.
    #[test]
    fn every_catalog_style_constructs() {
        for (name, _) in CATALOG_STYLES {
            let b = Bibliographer::new(name, BIBTEX, SourceFormat::Bibtex, None);
            assert!(b.is_ok(), "catalog style {name} failed: {:?}", b.err());
        }
    }
